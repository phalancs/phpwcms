			...
			*/
			...
			function Image( $file, $x, $y, $w = 0, $h = 0, $type = '', $link = '', $paint = true )
			{
				...
				if ( !isset( $this->images[$file] ) ) {
					...
					// Edited by Oliver Georgi, 2007-04-30 to better handle image type
					// you often see file extensions like *.php?do and so on...
					// with old style checking it will always fail
					if ( $type == '' ) {
				...
				if ( ( $y + $h ) > $this->fh ) {
					$this->AddPage();
					// Edited Oliver Georgi 2007-04-30 -> missing $this->tMargin
			...
			{
				if ( $black && $white ) $s = sprintf( '[%.3f %.3f] 0 d', $black * $this->k, $white * $this->k );
				else $s = '[] 0 d';
				$this->_out( $s );
			}
