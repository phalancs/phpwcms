...
<!--RSSFEED_START//-->[TITLE]<h3>{TITLE}</h3>[/TITLE][SUBTITLE]<h4>{SUBTITLE}</h4>[/SUBTITLE]<div>{FEEDINFO}{ITEMS}</div><!--RSSFEED_END//-->';
	...
	// Load SimplePie
	require_once(PHPWCMS_ROOT.'/include/ext/SimplePie/simplepie.inc.php');
	//require_once(PHPWCMS_ROOT.'/include/ext/SimplePie/idn/idna_convert.class.php');
	...
	*/
	...
	if( $rss_obj->data ) {
		...
		// whole rss feed
		$rss['template_RSSFEED'] = render_cnt_template($rss['template_RSSFEED'], 'TITLE', html($crow['acontent_title']) );
		$rss['template_RSSFEED'] = render_cnt_template($rss['template_RSSFEED'], 'SUBTITLE', html($crow['acontent_subtitle']) );
