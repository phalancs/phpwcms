...
if( (isset($_GET["s"]) && intval($_GET["s"]) == 1) || isset($_GET['struct']) ) { //Show single article information
	...
	if( (!isset($_GET["aktion"]) || !intval($_GET["aktion"])) && !isset($_GET['struct'])) {
		...
		include_once PHPWCMS_ROOT."/include/tmpl/articlecontent.list.tmpl.php";
	...
	// edit article summary
		...
		include_once PHPWCMS_ROOT."/include/tmpl/article.editsummary.tmpl.php";
		...
		if(isset($_GET["acid"]) && intval($_GET["acid"])) {
			...
			if($result = mysql_query($sql, $db) or die("error while reading article content data")) {
				if($row = mysql_fetch_array($result)) {
					...
					if($content["type"] != 30 && is_file(PHPWCMS_ROOT.'/include/lib/content/cnt'.$content["type"].'.takeval.inc.php')) {
					
						include(PHPWCMS_ROOT.'/include/lib/content/cnt'.$content["type"].'.takeval.inc.php');
						...
						include(PHPWCMS_ROOT.'/include/lib/content/cnt0.takeval.inc.php');
		...
		if(isset($_POST["caktion"]) && intval($_POST["caktion"])) {
			...
			include_once(PHPWCMS_ROOT."/include/lib/article.readform.inc.php"); //get posted values from form
			...
			if(!isset($content["error"])) { //if no error
				...
				if($content['type'] != 30 && file_exists(PHPWCMS_ROOT.'/include/lib/content/cnt'.$content['type'].'.sql.inc.php')) {
				
					include(PHPWCMS_ROOT.'/include/lib/content/cnt'.$content['type'].'.sql.inc.php');
					...
					include(PHPWCMS_ROOT.'/include/lib/content/cnt0.sql.inc.php');
		...
		include(PHPWCMS_ROOT."/include/tmpl/articlecontent.edit.tmpl.php");				
