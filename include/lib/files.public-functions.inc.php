...
function list_public($pid, $dbcon, $vor, $zieldatei, $userID, $show_thumb=1, $phpwcms) {
	...
	while($row = mysql_fetch_array($result)) {
		...
		//Aufbau der Zeile
		echo "<tr bgcolor=\"#EBF2F4\"><td colspan=\"2\"><img src=\"include/img/leer.gif\" height=\"1\" width=\"1\"></td></tr>\n"; //Abstand vor
		echo "<tr bgcolor=\"#EBF2F4\">\n"; //Einleitung Tabellenzeile
		echo "<td width=\"488\" class='msglist'>"; //Einleiten der Tabellenzelle
		echo $count."<img src=\"include/img/leer.gif\" height=\"1\" width=\"".($vor+6)."\" border=\"0\"><img src=\"include/img/icons/folder_zu.gif\" border=\"0\">"; //Zellinhalt 1. Spalte
		...
		if(!$klapp_status && $count_wert) { //$vor."<img src='include/img/leer.gif' height=1 width=18 border=0>"
			...
			if($file_result = mysql_query($file_sql, $dbcon) or die ("error while listing files")) {
				...
				while($file_row = mysql_fetch_array($file_result)) {
					...
					echo "<tr>\n";
					echo "<td width=\"".($vor+37)."\" class=\"msglist\"><img src=\"include/img/leer.gif\" height=\"1\" width=\"".($vor+37)."\" border=\"0\"></td>\n";
					...
					echo "></td>\n";
					echo "<td width=\"".(473-$vor)."\" class=\"msglist\"><img src=\"include/img/leer.gif\" height=\"1\" width=\"5\">"; //438-$vor
					...
					if($_SESSION["wcs_user_thumb"]) {
						...
						if($thumb_image != false) {
							...
							echo "<tr>\n";
							echo "<td width=\"".($vor+37)."\"><img src=\"include/img/leer.gif\" height=\"1\" width=\"".($vor+37)."\" border=\"0\"></td>\n";
							echo "<td width=\"13\"><img src=\"include/img/leer.gif\" height=\"1\" width=\"1\" border=\"0\"></td>\n<td width=\"";
							echo (473-$vor)."\"><img src=\"include/img/leer.gif\" height=\"1\" width=\"6\"><a href=\"fileinfo.php?public&amp;fid=";
...
function on_off($wert, $string, $art = 1) {
	...
	if($wert) {
		return ($art == 1) ? "+" : "<img src=\"include/img/symbols/klapp_zu.gif\" border=\"0\" title=\"".$GLOBALS['BL']['be_fprivfunc_opendir']." ".$string."\">";
	} else {
		return ($art == 1) ? "-" : "<img src=\"include/img/symbols/klapp_auf.gif\" border=\"0\" title=\"".$GLOBALS['BL']['be_fprivfunc_closedir']." ".$string."\">";
	}
}
