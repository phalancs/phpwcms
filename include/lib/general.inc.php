...
if(PHPWCMS_CHARSET == 'utf-8') {
	require_once (PHPWCMS_ROOT.'/include/lib/lib.php_special_entities.utf-8.php');
} else {
	require_once (PHPWCMS_ROOT.'/include/lib/lib.php_special_entities.php');
}
require_once (PHPWCMS_ROOT.'/include/lib/charset_helper.inc.php');
require_once (PHPWCMS_ROOT.'/include/ext/htmlfilter/htmlfilter.php');
require_once (PHPWCMS_ROOT.'/include/lib/helper.inc.php');
require_once (PHPWCMS_ROOT.'/include/ext/rfc822.php');
require_once (PHPWCMS_ROOT.'/include/ext/idna_convert/idna_convert.class.php');
...
function sendEmail($data = array(	'recipient'=>'','toName'=>'','subject'=>'','isHTML'=>0,'html'=>'','text'=>'',
	...
	if(count($sendTo)) {
		...
		include_once(PHPWCMS_ROOT.'/include/ext/phpmailer/class.phpmailer.php');
...
function makeCharsetConversion($string='', $in_charset='utf-8', $out_charset='utf-8', $entityEncode=false) {
	...
	if(!isset($phpwcms['convert_charsets'][$convertInOut])) {
		require_once (PHPWCMS_ROOT.'/include/ext/ConvertCharset/ConvertCharset.class.php');
...
function phpwcms_mkdir($target) {
	...
	if (phpwcms_mkdir(dirname($target))) {	// If the above failed, attempt to create the parent node, then try again.
		return phpwcms_mkdir($target);
	}
	return false;
}
