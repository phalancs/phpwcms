...
if(!_dbQuery($sql[0], 'CREATE')) {
	...
	$sql_error[0] = '<p class="error">Error creating <b>banner ads formats</b> initial database table: '.html(@mysql_error()).'</p>';

}
if(!_dbQuery($sql[1], 'CREATE')) {

	$sql_error[1] = '<p class="error">Error creating <b>banner ads campaign</b> initial database table: '.html(@mysql_error()).'</p>';

}
if(!_dbQuery($sql[2], 'CREATE')) {

	$sql_error[2] = '<p class="error">Error creating <b>banner ads tracking</b> initial database table: '.html(@mysql_error()).'</p>';

}
if(!_dbQuery($sql[3], 'CREATE')) {

	$sql_error[3] = '<p class="error">Error creating <b>banner ad place</b> initial database table: '.html(@mysql_error()).'</p>';
...
if(!isset($sql_error[0]) && !_dbQuery("SELECT COUNT(*) FROM ".DB_PREPEND."phpwcms_ads_formats", 'COUNT')) {
	...
	if(@mysql_error()) {
		$sql_error[4] = '<p class="error">Error inserting default <b>banner ads formats</b> entries: '.html(@mysql_error()).'</p>';	
	}
}
